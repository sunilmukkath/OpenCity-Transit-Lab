...
export function AnalyticsDashboard() {
  ...
  const showOnlyCategoryOnMap = (cat: CategoryFilter) => {
    ...
    setMapLayers((prev) => {
      ...
      // keep a sensible default when "all"
      if (cat === "all") {
        return {
          wards: true,
          zones: false,
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <MetricCard
          label="Realtime plugs"
          value={manifest.realtime.filter((r) => r.status === "not_connected").length}
          subtext="Not connected until agency feeds are wired"
        />
      </div>
        ...
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 font-[family-name:var(--font-display)] text-lg font-semibold text-[var(--ink)]">
              Slice sources, layers, and the map
            </h2>
          </div>
          <button
            type="button"
